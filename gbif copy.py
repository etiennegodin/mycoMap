...
def gbif_occurences_request(specie):
    
    if not os.path.exists(specie.request_key_path):
            
        #Create json query to send to gbif, also writes json
        query = construct_gbif_occurence_query(specie,decimal_longitude,decimal_latitude, specie.path )
        print(f"Starting API request for {specie}")
        await downloadQuery = occ.download(queries= query, format= 'SIMPLE_CSV', user = 'egodin', pwd = '4AWkTW8_4D$8q7.', email = 'etiennegodin@duck.com', pred_type='and')
        ...
        return specie.request_key_path, request_key
